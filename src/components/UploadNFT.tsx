...
import Fire from "../FireBase/FireBaseStore";
...
function UploadNFT() {
  ...
  const submitMintFile = async (e: any) => {
    e.preventDefault();
    try {
      if (typeof window.ethereum !== "undefined") {
        await activate();
        ...
        const signer = connection.provider?.getSigner();
        const contract = new ethers.Contract(BLUE_NFT_ADDRESS, BlueABI, signer);
        const fire = new Fire();
        const url = await fire.uploadImage(file);
        console.log(url);

        const Mint = await contract.mint(url);
        await Mint.wait();
        toast.success("successful deposit");
        console.log("Mint", Mint);
        navigate("/userNFT");
  ...
  return (
    <>
      ...
      <section className="py-6">
        ...
        <button onClick={submitMintFile}>upload</button>
      </section>
    </>
  );
}
